...
$(function() {
	$('.oks').click(function() {
		...
		$.ajax({
			type:'post',
			url:'../admin/adminPageReserveOk.do',
			data:{'rno':rno},
			success:location.href="../admin/adminPageReserve.do	"
		})
	})
})
</script>
     ...
     <c:forEach var="vo" items="${recvList }">
       <tr>
	      ...
	      <td class="text-center inline">
	       <c:if test="${vo.isok=='y' }">
	        <span class="btn btn-success btn-xs">승인완료</span>
	       </c:if>
	       <c:if test="${vo.isok=='n' }">
	        <span class="btn btn-default btn-xs " data-no=${vo.rno}>승인대기</span>
	       </c:if>
	        <input type=button class="btn-xs btn-danger oks" value="승인">
	      </td>
       </tr>
     </c:forEach>
    </table>
