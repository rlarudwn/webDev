...
<script type="text/javascript">
	$(function() {
		$('.infos').click(function() {
			...
			$.ajax({
				type : 'post',
				url : '../mypage/mypageRserveInfo.do',
				data : {
					'rno' : rno
				},
					...
					<c:forEach var="vo" items="${recvList }">
						<tr>
							...
							<td class="text-center inline">
								<c:if test="${vo.isok=='y' }">
									<span class="btn btn-success btn-xs infos">예약완료</span>
								</c:if>
								<c:if test="${vo.isok=='n' }">
									<span class="btn btn-default btn-xs">예약대기</span>
								</c:if>
								<input type=button class="btn-xs btn-danger" data-rno="${vo.rno}" value="취소">
							</td>
						</tr>
					</c:forEach>
				</table>
				<div style="height: 20px;"></div>
				<table class="table" id="info" style="display: none;">
					<caption>
						<h4>예약 정보</h4>
					</caption>
					<tr>
						<td width="30%" class="text-center" rowspan="8">
							<img src="" style="widows: 100%" id="poster">
						</td>
						<td colspan="2">
							<h3>
								<span id="name"></span>&nbsp;<span id="score" style="color: orange;"></span>
							</h3>
						</td>
					</tr>
					<tr>
